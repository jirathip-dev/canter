...
| `run release --run RUN_ID --reason TEXT [--idempotency-key IK] [--socket PATH] [--config PATH] [--json]` | Release ONE run that can never progress: its durable issue ownership and the occupancy it held are freed and the run becomes terminal (`invalidated`, never resumed/retried/dispatched again), with the reason, the exact run identity and the authorization window it held recorded in the audit. It refuses typed while a step of the run is in flight (`refusal.run.in_flight`) or while it still holds an unconsumed bounded retry authorization (`refusal.run.retry_pending` — never burned), replays only under the same idempotency key and refuses `refusal.run.terminal` otherwise. An expired/revoked/missing grant is not a fence (that is the case it exists for): the record states that window without presenting or reusing it, so continuing the work needs a freshly minted window. |
