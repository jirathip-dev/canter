...
state schema version: 13
migration chain: m0001_initial_state_v1, m0002_workflow_engine_instances_v2, m0003_control_plane_evidence_v3, m0004_schedules_lifecycle_v4, m0005_lane_replacements_v5, m0006_lane_checkpoints_v6, m0007_lane_successors_v7, m0008_lane_replacement_profiles_v8, m0009_queue_submissions_v9, m0010_run_controls_v10, m0011_supervision_v11, m0012_queue_advances_v12, m0013_supervision_retirement_v13
...
facts a build binds (`state schema version: 13`; migration chain m0001–m0013;
...
| `run retry --run RUN_ID --step STEP [--idempotency-key IK] [--socket PATH] [--config PATH] [--json]` | Authorize ONE bounded re-dispatch of ONE diagnosed step of the run (invalid/revoked/stale/succeeded/exhausted retries refuse). Authorization ONLY: the request dispatches nothing and spawns nothing — the authorization is consumed by the ONE re-dispatch of that exact step, which the run's own armed supervision performs (issue #241) or the operator's corrected dispatch consumes when it arrives first (single use: a second attempt needs its own authorization). |
| `queue intake --repository KEY --harness KEY --host HOST --caps G/R/H [--label L] [--pin N=HEX40]... [--max-items N] [--dry-run] [--out FILE] [--integration-branch B] [--socket PATH] [--config PATH] [--json]` | Turn the repository's own issue state into ONE bound-input submission, deterministically and idempotently (issue #245): the open issues carrying the ready label are selected in ascending issue-number order, revisions resolve by the documented rule (a tracker-declared `--pin` wins; otherwise the integration head read from the remote), an issue already owned or queued is never re-submitted (`intake.owned`), items beyond `--max-items` wait (`intake.cap`) instead of bypassing admission, an unresolvable revision refuses typed (`refusal.intake.revision`) and submits nothing, and two invocations over unchanged state produce the same digest. `--dry-run` prints the exact decision and mutates nothing. Contract: [docs/contracts/spec-intake.md](docs/contracts/spec-intake.md). |
| `run dispatch --run RUN_ID --step STEP [--param KEY=VALUE]... [--idempotency-key IK] [--socket PATH] [--config PATH] [--json]` | Dispatch ONE committed-spine step of the run with ONLY the step-specific inputs the operator knows (`--param`, merged over the committed params): the plan document, the issue/grant pins, the topology and the admission inputs are derived daemon-side from the run's committed submission and its own recorded dispatch context, the merged params are validated against the step kind's param contract before anything is journaled — a pre-screen that is total over the step kinds, so a malformed request of ANY kind refuses typed and burns no authorization, and a re-dispatch of a diagnosed step consumes exactly one unconsumed `run retry` authorization. |
...
| `run release --run RUN_ID --reason TEXT [--idempotency-key IK] [--socket PATH] [--config PATH] [--json]` | Release ONE run that can never progress: its durable issue ownership and the occupancy it held are freed and the run is terminal afterward (`done` stays `done`, otherwise `invalidated`; never resumed/retried/dispatched again), with the reason, the exact run identity and the authorization window it held recorded in the audit. It refuses typed while a step of the run is in flight (`refusal.run.in_flight`) or while it still holds an unconsumed bounded retry authorization (`refusal.run.retry_pending` — never burned, and the refusal names the remedy: the authorization is spent by the ONE re-dispatch of that exact step, which the run's own armed supervision performs or `run dispatch` consumes). Terminal runs (`done` or `invalidated`) can release leftover ownership without touching any successor's ownership. The same idempotency key replays the recorded response; a fresh key records an audited ownership-absent no-op if ownership is already freed. An expired/revoked/missing grant is not a fence (that is the case it exists for): the record states that window without presenting or reusing it, so continuing the work needs a freshly minted window. |
| `run retire-lane --run RUN_ID --reason TEXT [--topology FILE] [--idempotency-key IK] [--socket PATH] [--config PATH] [--json]` | Retire the stale LANE RECORDS of ONE run the ledger already records as terminal (issue #236): the leftover issue-ownership rows that still name it are removed in one audited transaction, and its lane residue is retired under the same #190/#222 policy — the run's OWN linked lane workspace, its registered lane checkout, and its local lane branch only when the published branch carries the same tip (this control never destroys a local-only delivery; the automatic reclaim at the next bind step reclaims an UNHELD one, issue #306). The lane is derived from the run's own issue and the integration clone from the run's own recorded topology (a first-time topology may be presented). A run that is NOT terminal refuses typed `refusal.lane.live_run` with nothing claimed or touched — a live lane still holds its issue's unique ownership (release it first when it can never progress); an unknown run is `state.not_found`; a terminal run that holds nothing left is an audited no-op; the same idempotency key replays the recorded response. |
| `supervision status --run RUN_ID [--socket PATH] [--config PATH] [--json]` | Read one supervised run's versioned `hf-supervision/v1` status back read-only (class/reason/eligibility, freshness, last check, next eligible check, and the frontier's retry disposition `evaluation.retry` — `awaiting-authorization` / `authorized-awaiting-dispatch` / `driver-dispatch` / `exhausted`, never confusable with a recorded `supervision.dispatch_refused`; supervision is armed by `queue submit --supervise arm`; a verified delivery each advances the authorized queue once and admits the next eligible approved issue under the existing admission checks, and nothing else is ever continued). |
