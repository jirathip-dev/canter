...
| Apply refused | `refusal.admission.proof_stale` | A LIVE run's own dispatch renews its lapsed host-resource proof at dispatch time (audited `host.proof.renewal`, measured at the run's lane root): the supervisor's continuation AND the run's own bounded check re-evaluation (`canter run reevaluate`, issue #243) both re-measure, so nothing to do while the lane root is exposed. When the host cannot be measured (`run.host_proof.unmeasurable` on the daemon log) the refusal stands: restore the lane root the topology names, or present a fresh attestation (`run dispatch --admission FILE` — or `run reevaluate` for the run's own terminal-success check producer). The refusal itself names the precondition and these exact commands. |
| Apply refused | `refusal.admission.resource_floor` | The host the run's lane root lives on exposes fewer free bytes than the documented floor (`HOST_FREE_FLOOR_BYTES`), so the lane start is refused BEFORE anything is spawned: the refusal names the observation and the floor. Reclaim regenerable lane residue (the reaper does this for a terminal generation's `<lane>-derived`/`<lane>-DD` scratch roots under the run's worktrees root) or point `topology.worktrees_root` at a volume with room; the floor is a design commitment and is never bypassed. |
...
| Review refused | `refusal.evidence.verdict_stale` (the lane checkout moved past the certified head — before the reviewer started, or while the review was open) | The certificate is a recorded fact and the checkout's movement is external, so the step can never be repaired by a re-dispatch: it parks typed with its bounded retries UNSPENT (issue #200) and a stale dispatch of it refuses `refusal.run.retry_required`. The reviewed work is fenced for the whole review window (issue #207): a commit that lands mid-review is refused at consumption exactly like one that predates the reviewer, and the delivery must re-enter review. Repair the lane checkout to the certified head, then authorize that exact step (`run retry`): the run's own armed supervision consumes the authorization by re-dispatching it (issue #241), or dispatch it yourself (`run dispatch`) — or re-run the issue. Never move the certificate to the newer head. |
...
| Merge refused | `refusal.delivery.moved` (a commit landed on the reviewed delivery after the verdict) | The verdict is a recorded fact and the delivery's movement is external, so the step can never be repaired by a re-dispatch: it parks typed with its bounded retries UNSPENT (issue #202). The delivery must re-enter review and a NEW verdict must name the moved head before any step consumes it — never point the certificate at the newer head. |
| Merge refused | `effect.merge.base_moved` (the published integration ref moved past the base the delivery certified and cannot carry the certified content byte-identically — the refresh conflicts, or it rewrites paths the review covered) | A base move is a recorded fact about the PUBLISHED ref, not a step defect: the refresh is withdrawn (the delivery is left at the exact head its verdict names) and the step parks typed with its bounded retries UNSPENT (issue #263) — never a terminal `effect.merge.failed` that spends the whole budget on a condition the run cannot resolve by itself. Refresh the delivery against the current published head under a fresh review: no step consumes content no review covered, so a NEW verdict must name the refreshed head before it is landed. |
| Review/merge refused | `refusal.delivery.unbound` (no collection of this run certified the head) | The run's own `collect_outcome` step has not certified a deliverable head (or a step presents a different one). Re-run the run's own collection: a head no collection observed is never reviewed and never landed. |
| Collection refused | `refusal.collect.unbound` | The collection could not bind the 40-hex head and the slug branch it observed; nothing was certified and nothing is consumed. Fix the lane checkout (branch + HEAD) and re-run the collection. |
